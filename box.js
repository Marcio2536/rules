...
// 存儲`全域備份` (棄用, 改用 `chavy_boxjs_backups`)
  ...
  const sysapps = [
    {
      id: 'BoxSetting',
      name: '偏好設置',
      descs: ['可手動執行一些抹掉資料的腳本', '可設置明暗兩種主題下的主色調', '可設置壁紙清單'],
      keys: [
      ...
      settings: [
        { id: '@chavy_boxjs_userCfgs.httpapis', name: 'HTTP-API (Surge)', val: '', type: 'textarea', placeholder: ',examplekey@127.0.0.1:6166', autoGrow: true, rows: 2, persistentHint:true, desc: '示例: ,examplekey@127.0.0.1:6166! 注意: 以逗號開頭, 逗號分隔多個位址, 可加回車' },
        ...
        { id: '@chavy_boxjs_userCfgs.changeBgImgEnterDefault', name: '手勢進入壁紙模式預設背景圖片', val: '', type: 'text', placeholder: '填寫上面背景圖片清單的值', persistentHint:true, desc: '' },
        { id: '@chavy_boxjs_userCfgs.changeBgImgOutDefault', name: '手勢退出壁紙模式預設背景圖片', val: '', type: 'text', placeholder: '填寫上面背景圖片清單的值', persistentHint:true, desc: '' },
 ...
 *
 * 升級前: 把所有備份都存到一個持久化空間
 * 升級後: 把每個備份都獨立存到一個空間, `$.KEY_backups` 僅記錄必要的資料索引
 */
