...
let result = {
  "title": '    📺  流媒體服務查詢',
  "YouTube": '<b>YouTube: </b>檢測失敗，請重試',
  "Netflix": '<b>Netflix: </b>檢測失敗，請重試',
  "Dazn": "<b>Dazn: </b>檢測失敗，請重試",
  "Disney": "<b>Disney⁺: </b>檢測失敗，請重試",
...
;(async () => {
  ...
  if (status==STATUS_COMING) {
    //console.log(1)
    result["Disney"] = "<b>Disney⁺:</b> 即將登陸 ➟ "+'⟦'+flags.get(region.toUpperCase())+"⟧"
    ...
    result["Disney"] = "<b>Disney⁺:</b> 測試超時 "
  }
  
  let content = "------------------------------"+"</br>"+([result["YouTube"],result["Netflix"],result["Disney"],result["Dazn"]]).join("</br></br>")
  content = content + "</br>------------------------------</br>"+"<font color=#6959CD>"+"<b>節點</b> ➟ " + $environment.params+ "</font>"
  ...
  return new Promise((resolve, reject) =>{
    ...
    $task.fetch(option).then(response => {
      ...
      if (response.statusCode === 404) {
        ...
        result["Netflix"] = "<b>Netflix: </b>支持自制劇集"
...
function testYTB() { 
    ...
    $task.fetch(option).then(response=> {
      ...
      if (response.statusCode !== 200) {
        //reject('Error')
        result["YouTube"] = "<b>YouTube Premium: </b>檢測失敗"
