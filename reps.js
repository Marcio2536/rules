...
⦿ 𝗿𝗲𝗻𝗮𝗺𝗲 重命名, "舊名@新名", "首碼@", "@尾碼", 用 "+" 連接多個參數;
  ...
  ❖ $type0/1/2/3/4/5 預留位置，將節點類型(ss/ssr/vmess 等)作為可指令引數，如
    ∎ 𝐫𝐞𝐧𝐚𝐦𝐞=@|$type2
    ∎ 樣式分別為 "𝐬𝐬","𝐒𝐒","🅢🅢","🆂🆂","ⓢⓢ","🅂🅂"
  ❖ $emoji1/2 預留位置，將節點地區emoji(🇭🇰 🇯🇵 等)作為可指令引數，如
    ∎ 𝐫𝐞𝐧𝐚𝐦𝐞=@「$emoji1」
⦿ 𝘀𝘂𝗳𝗳𝗶𝘅=-1/1 將節點類型做為首碼/尾碼 添加在節點名中, 如 「𝗌𝗌」 「𝖵𝗆𝖾𝗌𝗌」
...
⦿ ⟦進階參數⟧: 𝘀𝗳𝗶𝗹𝘁𝗲𝗿/𝘀𝗿𝗲𝗻𝗮𝗺𝗲, 傳入一段 base64 編碼的腳本, 可用於更為複雜的[過濾/重命名] 需求
...
⦿ 類型參數 𝐭𝐲𝐩𝐞=𝐝𝐨𝐦𝐚𝐢𝐧-𝐬𝐞𝐭/𝐫𝐮𝐥𝐞/𝐦𝐨𝐝𝐮𝐥𝐞/𝐥𝐢𝐬𝐭/𝐧𝐨𝐝𝐞𝐬
  ❖ 當解析器未能正確識別類型時, 可嘗試使用此參數強制指定
⦿ 隱藏參數 hide=1, 隱藏篩除的分流/重寫，預設方式為禁用
----------------------------------------------------------
*/
...
var para1 = para.slice(para.indexOf("#") + 1).replace(/\$type/g,"node_type_para_prefix").replace(/\$emoji/g,"node_emoji_flag_prefix") //防止參數中其它位置也存在"#"
...
var Preplace = mark0 && para1.indexOf("replace=") != -1 ? para1.split("replace=")[1].split("&")[0] : null; //filter/rewrite 正則替換
...
function Parser() {
  try {
    type0 = Type_Check(content0); //  類型判斷
    //$notify(type0,"hh")
    total = ResourceParse();
    
  } catch (err) {
    $notify("❌ 解析出現錯誤", "⚠️ 請點擊發送連結回饋", err, bug_link);
  }
  //$notify("","",total)
  //$done({ content: total });
}
...
function ParseUnkown(cnt){
  try {
    cnt = JSON.parse(cnt)
    if(cnt) {
      $notify("⚠️ 連結返回內容並非有效訂閱"+ "⟦" + subtag + "⟧","⁉️ 請自行檢查原始連結，返回內容 👇️👇️",JSON.stringify(cnt), bug_link)
    }
    
  } catch(err) {
    $notify("😭 未能識別該訂閱格式：  " + "⟦" + subtag + "⟧",  "⚠️ 將直接導入Quantumult X \n 如認為是 BUG, 請點通知跳轉回饋", "連結返回內容:\n"+cnt, bug_link);
  }
}
...
function ResourceParse() {
  ...
  if (flag == 1) { //server 類型統一處理
    ...
    if (total.length > 0){
      ...
      if (Pcnt == 1) {$notify("解析後最終返回內容" , "節點數量: " +total.split("\n").length, total)}
...
//  } else if (Pinfo ==1){
//    $notify("流量資訊: ⟦" + subtag + "⟧", "", "⚠️ 該訂閱連結未返回任何流量資訊", subinfo_link)
//  }
}
...
function flowcheck(cnt) {
    for (var i = 0; i < cnt.length; i++) {
        ...
        if (item.indexOf("剩餘流量") != -1) {
            flow = nm
        } else if (item.indexOf("期時間") != -1) {
            exptime = nm
        }
    }
  flow = flow? flow:"⚠️ 該訂閱未返回任何流量資訊"
  exptime = exptime? exptime:"⚠️ 該訂閱未返回套餐時間資訊"
...
function Type_Check(subs) {
    ...
    if (subs.indexOf(html) != -1 && link0.indexOf("github.com" == -1)) {
      ...
      type = "rewrite" //Quantumult X 類型 rewrite/ Surge Script/
    } else if ( (((ModuleK.some(RewriteCheck) || para1.indexOf("dst=rewrite") != -1) && (para1.indexOf("dst=filter") == -1) && subs.indexOf("[Proxy]") == -1) || typeU == "module") && typeU != "nodes") { // Surge 類型 module /rule-set(含url-regex) 類型
      type = "sgmodule"
    ...
    } else if (subsn.length >= 1 && SubK2.some(NodeCheck) && !/\[(Proxy|filter_local)\]/.test(subs)) { //未b64加密的多行URI 組合訂閱
      type = "Subs"
...
function ToRaw(cnt) {
  ...
  if (cnt) {
    ...
    $notify( "⚠️⚠️ 解析該資源" + " ⟦" + subtag + "⟧ 失敗" , "🚥 你的連結似乎是目錄，而不是檔" , "❌ 你的連結："+link0, {"open-url":link0})
  }
  return cnt
}
...
function Subs2QX(subs, Pudp, Ptfo, Pcert, Ptls13) {
    ...
    if (failedList.length > 0 && Pntf0 != 0) {
        $notify(`⚠️ 有 ${failedList.length} 條資料解析出錯, 已跳過`, "出錯內容", failedList.join("\n"));
    }
    return QXlist;
}

//將sip008格式的訂閱轉換成quanx格式
...
function get_emoji(emojip, sname) {
    var Lmoji = { "🏳️‍🌈": ["流量", "時間", "應急", "過期", "Bandwidth", "expire"], "🇦🇹": ["奧地利", "Austria", "維也納"], "🇦🇺": ["AU", "Australia", "Sydney", "澳大利亞", "澳洲", "墨爾本", "悉尼" ,"土澳"], "🇧🇪": ["BE", "比利時"], "🇧🇬": ["保加利亞", "Bulgaria"], "🇧🇷": ["BR", "Brazil", "巴西", "聖保羅"], "🇨🇦": ["Canada","CANADA", "Waterloo", "加拿大", "蒙特利爾", "溫哥華", "楓葉", "楓葉", "滑鐵盧", "多倫多"], "🇨🇭": ["瑞士", "蘇黎世", "Switzerland"], "🇨🇿": ["Czechia", "捷克"], "🇩🇪": ["DE", "German", "GERMAN", "德國", "德國", "法蘭克福","京德"], "🇩🇰": ["丹麥"], "🇪🇸": ["ES", "西班牙", "Spain"], "🇪🇺": ["EU", "歐盟", "歐羅巴"], "🇫🇮": ["Finland", "芬蘭", "赫爾辛基"], "🇫🇷": ["FR", "France", "法國", "法國", "巴黎"], "🇬🇧": ["UK", "GB", "England", "United Kingdom", "英國", "倫敦", "英"], "🇲🇴": ["MO", "Macao", "澳門", "澳門", "CTM"], "🇰🇿": ["哈薩克"], "🇭🇺": ["匈牙利", "Hungary"], "🇭🇰": ["HK", "Hongkong", "Hong Kong", "HongKong", "HONG KONG","香港", "深港", "滬港", "呼港", "HKT", "HKBN", "HGC", "WTT", "CMI", "穗港", "京港", "港"], "🇮🇩": ["Indonesia", "印尼", "印尼", "雅加達"], "🇮🇪": ["Ireland", "IRELAND", "愛爾蘭", "愛爾蘭", "都柏林"], "🇮🇱": ["Israel", "以色列"], "🇮🇳": ["India", "INDIA","印度", "孟買", "Mumbai"], "🇰🇵": ["KP", "朝鮮"], "🇰🇷": ["KR", "Korea", "KOR", "韓國", "首爾", "韓", "韓"], "🇱🇻": ["Latvia", "Latvija", "拉脫維亞"], "🇲🇽️": ["MEX", "MX", "墨西哥"], "🇲🇾": ["MY", "Malaysia","MALAYSIA", "馬來西亞", "馬來西亞", "吉隆玻"], "🇳🇱": ["NL", "Netherlands", "荷蘭", "荷蘭", "尼德蘭", "阿姆斯特丹"], "🇵🇭": ["PH", "Philippines", "菲律賓", "菲律賓"], "🇷🇴": ["RO", "羅馬尼亞"], "🇷🇺": ["RU", "Russia", "俄羅斯", "俄國", "俄羅斯", "伯力", "莫斯科", "聖彼德堡", "西伯利亞", "新西伯利亞", "京俄", "杭俄"], "🇸🇦": ["沙特", "迪拜"], "🇸🇪": ["SE", "Sweden"], "🇸🇬": ["SG", "Singapore","SINGAPORE", "新加坡", "獅城", "滬新", "京新", "泉新", "穗新", "深新", "杭新", "廣新"], "🇹🇭": ["TH", "Thailand", "泰國", "泰國", "曼谷"], "🇹🇷": ["TR", "Turkey", "土耳其", "伊斯坦布爾"], "🇹🇼": ["TW", "Taiwan","TAIWAN", "臺灣", "臺北", "台中", "新北", "彰化", "CHT", "台", "HINET"], "🇺🇸": ["US", "USA", "America", "United States", "美國", "美", "京美", "波特蘭", "達拉斯", "俄勒岡", "鳳凰城", "費利蒙", "矽穀", "矽穀", "拉斯維加斯", "洛杉磯", "聖約瑟", "聖克拉拉", "西雅圖", "芝加哥", "滬美", "哥倫布", "紐約"], "🇻🇳": ["VN", "越南", "胡志明市"], "🇮🇹": ["Italy", "IT", "Nachash", "義大利", "米蘭", "義大利"], "🇿🇦": ["South Africa", "南非"], "🇦🇪": ["United Arab Emirates", "阿聯酋"], "🇯🇵": ["JP", "Japan","JAPAN", "日", "日本", "東京", "大阪", "埼玉", "滬日", "穗日", "川日", "中日", "泉日", "杭日", "深日", "遼日", "廣日"], "🇦🇷": ["AR", "阿根廷"], "🇳🇴": ["Norway", "挪威", "NO"], "🇨🇳": ["CN", "China", "回國", "中國", "江蘇", "北京", "上海", "廣州", "深圳", "杭州", "徐州", "青島", "寧波", "鎮江", "back"] }
